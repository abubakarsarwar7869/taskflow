...
export function Sidebar({ onOpenSettings }: SidebarProps) {
  ...
  const { user } = useAuth();
  const { boards, currentBoardId, setCurrentBoard, createBoard, deleteBoard } = useTaskStore();
  ...
  const accessibleBoards = boards.filter((board) => {
    ...
    // User is the owner
    if (board.ownerId === user?.id || board.ownerId === user?._id) return true;
    
    // User is in the members array
    const isMember = board.members.some(
      (member) => member.id === user?.id || member.id === user?._id || member.email === user?.email
    );
